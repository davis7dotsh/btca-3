...
import { BoxHybridAgentService } from "$lib/services/agent";
import {
  ...
  try {
    const stream = await runtime.runPromise(
      Effect.gen(function* () {
        ...
        const agent = yield* BoxHybridAgentService;
        ...
        const eventStream = Stream.succeed<AgentPromptStreamEvent>({
          ...
          Stream.concat(
            Stream.fromAsyncIterable(
              ...
              Stream.tap((agentEvent) =>
                Effect.sync(() => {
                  if (agentEvent.type === "agent_end") {
                    console.log("Box hybrid agent stream delivered to client", {
                      threadId,
              ...
              Stream.filter(
                (streamEvent): streamEvent is AgentPromptStreamEvent => streamEvent !== null,
              ),
            ),
          ),
    ...
    if (error instanceof AgentRequestError) {
      if (error.status === 401) {
        console.error("Unauthorized box hybrid agent request", {
    ...
    if (error instanceof BoxServiceError) {
      console.error("Failed to start box hybrid agent stream", {
